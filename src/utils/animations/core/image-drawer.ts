...
export class ImageDrawer<T extends ParticleBase & { size?: number, rotation?: number }> 
    implements ParticleDrawer<T> {
  ...
  public drawFrame(particles: ReadonlyArray<T>): void {
    ...
    for (const particle of particles) {
      ...
      if (!this.isValidParticle(particle)) continue;
      
      // Get image for this particle
      const imageIndex = this.particleAssetMap.get(particle.id) || 0;
  ...
  private isValidParticle(particle: T): boolean {
    return (
      particle !== null &&
